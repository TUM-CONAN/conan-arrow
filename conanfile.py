...
class ArrowConan(ConanFile):
    ...
    def _requires_rapidjson(self):
        return (
            self.options.with_json
            or self.options.encryption
            or (Version(self.version) >= "21.0.0" and self.options.parquet)
        )
    ...
    def requirements(self):
        ...
        if (
            self.options.simd_level != "disabled"
            or self.options.runtime_simd_level != "disabled"
        ):
    ...
    def validate(self):
        ...
        auto_options = [
            option for option, value in self.options.items() if value == "auto"
        ]
        if auto_options:
            raise ConanException(
                "Options with value 'auto' are deprecated. Please set them true/false or use its default value."
        ...
        if self.options.gandiva:
            if not self.options.with_re2:
                raise ConanException(
                    "'with_re2' option should be True when 'gandiva=True'"
                )
            if not self.options.with_boost:
                raise ConanException(
                    "'with_boost' option should be True when 'gandiva=True'"
                )
            if not self.options.with_utf8proc:
                raise ConanException(
                    "'with_utf8proc' option should be True when 'gandiva=True'"
                )
        if self.options.with_orc:
            ...
            if not self.options.with_snappy:
                raise ConanException(
                    "'with_snappy' option should be True when 'orc=True'"
                )
            if not self.options.with_zlib:
                raise ConanException(
                    "'with_zlib' option should be True when 'orc=True'"
                )
            if not self.options.with_zstd:
                raise ConanException(
                    "'with_zstd' option should be True when 'orc=True'"
                )
        if self.options.with_thrift and not self.options.with_boost:
            raise ConanException(
                "'with_boost' option should be True when 'thrift=True'"
            )
        if self.options.parquet:
            if not self.options.with_thrift:
                raise ConanException(
                    "'with_thrift' option should be True when 'parquet=True'"
                )
        if self.options.with_flight_rpc and not self.options.with_protobuf:
            raise ConanException(
                "'with_protobuf' option should be True when 'with_flight_rpc=True'"
            )
        ...
        if self.options.with_thrift and not self.options.with_zlib:
            raise ConanInvalidConfiguration(
                "arrow:with_thrift requires arrow:with_zlib"
            )
    ...
    def generate(self):
        ...
        tc.variables["ARROW_PACKAGE_KIND"] = (
        ...
        if self.options.with_grpc:
            tc.variables["ARROW_GRPC_USE_SHARED"] = bool(
                self.dependencies["grpc"].options.shared
            )
        ...
        if self.options.with_bz2:
            tc.variables["ARROW_BZ2_USE_SHARED"] = bool(
                self.dependencies["bzip2"].options.shared
            )
        tc.variables["ARROW_WITH_LZ4"] = bool(self.options.with_lz4)
        tc.variables["lz4_SOURCE"] = "SYSTEM"
        if self.options.with_lz4:
            tc.variables["ARROW_LZ4_USE_SHARED"] = bool(
                self.dependencies["lz4"].options.shared
            )
        ...
        tc.cache_variables["ARROW_SIMD_LEVEL"] = (
            str(simd_level).upper() if simd_level != "disabled" else "NONE"
        )
        tc.cache_variables["ARROW_RUNTIME_SIMD_LEVEL"] = (
            str(runtime_simd_level).upper()
            if runtime_simd_level != "disabled"
            else "NONE"
        )
        if self.options.with_zstd:
            tc.variables["ARROW_ZSTD_USE_SHARED"] = bool(
                self.dependencies["zstd"].options.shared
            )
        ...
        if self.options.with_openssl:
            tc.variables["OPENSSL_ROOT_DIR"] = self.dependencies[
                "openssl"
        ...
        if self.options.with_boost:
            ...
            tc.variables["ARROW_BOOST_USE_SHARED"] = bool(
                self.dependencies["boost"].options.get_safe("shared", False)
            )
        ...
        if is_msvc(self):
            tc.variables["ARROW_USE_STATIC_CRT"] = is_msvc_static_runtime(self)
        if self.options.with_llvm:
            tc.variables["LLVM_DIR"] = self.dependencies[
                "llvm-core"
            ].package_folder.replace("\\", "/")

        tc.cache_variables["CMAKE_PROJECT_arrow_INCLUDE"] = os.path.join(
    ...
    def package_info(self):
        ...
        self.cpp_info.components["libarrow"].set_property(
            "cmake_target_name", f"Arrow::arrow_{cmake_suffix}"
        )
        ...
        if self.options.parquet:
            ...
            self.cpp_info.components["libparquet"].set_property(
                "cmake_target_name", f"Parquet::parquet_{cmake_suffix}"
            )
        ...
        if self.options.get_safe("substrait"):
            ...
            self.cpp_info.components["libarrow_substrait"].set_property(
                "cmake_target_name", f"Arrow::arrow_substrait_{cmake_suffix}"
            )
            ...
            if self.options.dataset_modules:
                self.cpp_info.components["libarrow_substrait"].requires.append(
                    "dataset"
                )
        ...
        if self.options.acero:
            self.cpp_info.components["libacero"].set_property(
                "pkg_config_name", "acero"
            )
            self.cpp_info.components["libacero"].set_property(
                "cmake_target_name", f"ArrowAcero::arrow_acero_{cmake_suffix}"
            )
        ...
        if Version(self.version) >= "21.0.0" and self.options.compute:
            self.cpp_info.components["libarrow_compute"].set_property(
                "pkg_config_name", "arrow_compute"
            )
            self.cpp_info.components["libarrow_compute"].set_property(
                "cmake_target_name", f"ArrowCompute::arrow_compute_{cmake_suffix}"
            )
            self.cpp_info.components["libarrow_compute"].libs = [
                f"arrow_compute{suffix}"
            ]
            self.cpp_info.components["libarrow_compute"].requires = ["libarrow"]

        if self.options.gandiva:
            self.cpp_info.components["libgandiva"].set_property(
                "pkg_config_name", "gandiva"
            )
            self.cpp_info.components["libgandiva"].set_property(
                "cmake_target_name", f"Gandiva::gandiva_{cmake_suffix}"
            )
        ...
        if self.options.with_flight_rpc:
            ...
            self.cpp_info.components["libarrow_flight"].set_property(
                "cmake_target_name", f"ArrowFlight::arrow_flight_{cmake_suffix}"
            )
        ...
        if self.options.get_safe("with_flight_sql"):
            ...
            self.cpp_info.components["libarrow_flight_sql"].set_property(
                "cmake_target_name", f"ArrowFlightSql::arrow_flight_sql_{cmake_suffix}"
            )
        ...
        if self.options.with_boost:
            ...
            if (
                ...
                and Version(self.settings.compiler.version) < Version("4.9")
            ):
        ...
        if self.options.with_s3:
            ...
            self.cpp_info.components["libarrow"].requires.extend(
                [
                    f"aws-sdk-cpp::{x}"
                    for x in [
                        "cognito-identity",
